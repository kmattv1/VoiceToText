...
class DroopyFieldStorage(cgi.FieldStorage):
    ...
    def make_file(self, binary=None):
        ...
        fd, name = tempfile.mkstemp(dir=self.directory + '/uploaded', prefix=self.TMPPREFIX)
    ...
    def do_POST(self):
        ...
        try:
            ...
            form = DroopyFieldStorage(fp=self.rfile,
                                      directory=self.directory + '/uploaded',
                                      headers=self.headers,
                                      environ={'REQUEST_METHOD': self.command})
            file_items = form.list
            ...
            for item in file_items:
                if (item.filename):
                    ...
                    if filename == "":
                        continue
                    localpath = _encode_str_if_py2(os.path.join(self.directory + '/uploaded', filename), "utf-8")
